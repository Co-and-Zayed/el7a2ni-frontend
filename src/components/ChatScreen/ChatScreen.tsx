...
const ChatScreen = () => {
  ...
  const recepientId: any =
    userData?._id === "6529674bf0f495495fcc73d1"
      ? "65296a487fc835c0cc4d21ae" // neveen
      : "6529674bf0f495495fcc73d1"; // seif
  ...
  const updateMessages = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.log("Updated messages fetched successfully:", updatedMessages);
  ...
  const renderInboxItem = (data: { name: string; lastMessage: string }) => {
    return (
      <div
        className={`${data?.name === currentChat && styles.activeChat} ${
          styles.inboxItem
              ...
              messages.map((msg: any, idx: any) => {
                ...
                return (
                  <>
                    {showDate && (
                      <motion.div
                        key={idx} // Add a unique key for each message
                        initial={{ opacity: 0, x: 50 * (isMe ? 1 : -1) }}
